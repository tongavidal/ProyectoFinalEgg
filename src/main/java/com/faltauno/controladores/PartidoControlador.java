...
public class PartidoControlador {
	...
	public String listarPartidosFiltrados(ModelMap modelo, @RequestParam String idLocalidad,
			@RequestParam(required = false) Sexo sexo) throws ErrorServicio {
		try {
			modelo.put("title", "Lista de Partidos - NosFalta1");
	...
	public String confirmarpostulado(ModelMap modelo, @RequestParam String idpartido, @RequestParam String idpostulado)
	...
	public String eliminarPostulado(ModelMap modelo, @PathVariable String idPartido, @PathVariable String idUsuario)
		...
		try {
			...
			List<Usuario> listaConfirmados = partidoServicio.listarConfirmados(partido);
			modelo.put("confirmados", listaConfirmados);
			modelo.put("mensajeexito", "El postulado fue cancelado con exito");
			return "listado-confirmados.html";
		} catch (ErrorServicio es) {
			modelo.put("error", es.getMessage());
			return ("listado-postulados");
	...
	public String postularse(ModelMap modelo, @RequestParam String idpartido, @RequestParam String idpostulado)
			throws ErrorServicio {
		...
		try {
			...
			// vuelvo a cargar postulados para mostrar
			// List<Usuario> postulados =
			// partidoRepositorio.findById(idpartido).get().getJugPostulados();
			// modelo.put("listado-postulados", postulados);
	...
	public String registroPartido(HttpSession session, @RequestParam String idcreador, ModelMap modelo,
			@RequestParam String idEstablecimiento, @RequestParam String idLocalidad,
			@RequestParam Integer cantJugadores, @RequestParam String horario, @RequestParam Integer cantVacantes,
			@RequestParam Double precio, @RequestParam Sexo sexo,
			@RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date fecha) {
		try {
			Usuario creador = usuarioRepositorio.getOne(idcreador);
			partidoServicio.crearPartido(idEstablecimiento, idLocalidad, cantJugadores,
					partidoServicio.horario(horario), cantVacantes, precio, creador, sexo, fecha);
		} catch (ErrorServicio ex) {
			List<Establecimiento> establecimientos = establecimientoServicio.listaEstablecimientos();
			modelo.put("establecimientos", establecimientos);
			List<Localidad> localidades = localidadServicio.listarTodasLocalidads();
	...
	public String modificarPartido(@PathVariable String id, @RequestParam String idEstablecimiento,
			@RequestParam String idLocalidad, ModelMap modelo, @RequestParam String horario, @RequestParam Sexo sexo,
			@RequestParam Double precio, @RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date fecha) {
		try {
			partidoServicio.modificarPartido(id, idLocalidad, idEstablecimiento, partidoServicio.horario(horario),
					precio, sexo, fecha);
	...
	public String verPartido(ModelMap modelo, @RequestParam(required = false) String idlocalidad,
			@RequestParam(required = false) String idpartido) throws ErrorServicio {
		try {
